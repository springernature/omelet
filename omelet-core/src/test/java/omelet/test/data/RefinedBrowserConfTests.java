...
import omelet.data.driverconf.RefinedBrowserConf;
...
public class RefinedBrowserConfTests {
	...
	public void ifCommandLineAndOtherPresent() {
		...
		RefinedBrowserConf rbf = new RefinedBrowserConf(clientMap,
				"test.properties");
		Assert.assertEquals(rbf.get("bsUserName", "defaultUserName"),
				commandLineValue);
	}
	...
	public void verifyFrameworkPropertiesValueSet() {
		...
		Map<String, String> clientMap = new HashMap<String, String>();
		IProperty frameworkProp = new PropertyMapping(Utils.getResources(this,
				dataFileName));
		RefinedBrowserConf rbf = new RefinedBrowserConf(clientMap, dataFileName);
		Assert.assertEquals(rbf.get(key, "defaultUserName"),
				frameworkProp.getValue(key));
	}
